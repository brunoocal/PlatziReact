...
class BadgeForm extends React.Component{
    ...
    handleProgressBarChange = (event) => {
        //Convierto el objeto con los valores de los formularios que me llega desde PROPS, IMPORTANTE, porque sino nunca tendríamos los valores de entrada del formulario.
        const list = Object.keys(this.props.formValues.progressBar).map((key) => [String(key), this.props.formValues.form[key]]);

        //formInput es el resultado dentro del array list, donde element === event.target.value, osea, que el elemento sea igual al valor que nos vino por el evento:
        //si este evento lo ejecuta el <Input name="firstName"/>, formInput será el array dentro de list, que tenga el valor de entrada de firstName
        const formInput = list.find(index => index.find(element => element === event.target.value) )
        //value es el valor que se le asignará a nuestro objeto controlador de la progressbar cada vez que sea llamado el evento, tengo 5 inputs, pero agregé 1 para la llamada al API (simulada)
        //entonces cada input tendría un valor total en la progressbar de 16.66667%
        //Si formInput está indefinido, se asigna 0, si no, luego se comprueba que el valor 1 en el array formInput, sea igual al parámentro de entrada del <Input/> que accionó el evento,
    ...
    renderProgressBar = () => {
        ...
        //Básicamente si están todas las casillas completas, y nuestro envio de datos a la API fue satisfactorio, se manda el mensaje que esté en this.progressBar.message
        //sino, el que ahí ustedes ven.
        //Si hubo algún fallo, como un error en nuestro API, o nuestro usuario no llenó todas las casillas, this.progressBar.error va a ser true,
        //por lo tanto, saldrá el mensaje ahí provisto, sino, dirá cuantas casillas faltan para llenar completamente el formulario
        message = (withoutData <= 0) ? (this.progressBar.success ? this.progressBar.message : "¡Tú Badge está lista, ¿No quedó genial?!") : (this.progressBar.error ? "¡Necesitas rellenar todos los datos!" : "¡Faltan " + withoutData + " casillas por completar!")
        //Basicamente lo mismo que arriba pero con los colores de variantes
        variant = (withoutData <= 0) ? this.progressBar.variant : (this.progressBar.error ? this.progressBar.variant : "warning" )
    ...
    submitApi = e => {
        ...
        if((this.progressBar.porcent / 16.66667) >= 5){
        //Ejecutamos la función que nos llega por props
        this.props.onSubmit();
        //TODO: Hacer luego READ con la API a la badge, confirmar error o acierto realmente, sin simulaciones
        ...
        this.progressBar = {
            ...
            this.progressBar = {
                ...
                message: "¡Tienes que rellenar todas las casillas!", //Mensaje de error
